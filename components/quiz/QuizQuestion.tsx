...
export function QuizQuestion({ item, onAnswer, onContinue, onRestart, answered }: QuizQuestionProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const correct = item.correctForms.some((form) => form.toLowerCase() === trimmedAnswer)
  ...
  return (
    <div className="flex flex-col w-full max-w-md mx-auto p-4 md:p-6 bg-white rounded-xl shadow-md">
      ...
      {showRestartConfirm && (
        <div
          className={`modal-container bg-black bg-opacity-50 p-4 ${isExitingModal ? "exiting" : ""}`}
        >
          <div
            className={`modal-content bg-white rounded-lg p-5 max-w-xs w-full shadow-lg ${isExitingModal ? "exiting" : ""}`}
          >
            <h3 className="text-lg font-bold text-gray-800 mb-3">Confirmar reinici</h3>
            <p className="text-gray-600 mb-4">
              Estas segur que vols reiniciar el quiz? Perdras tot el progres actual.
            </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {!submitted ? (
          ...
          <div className="space-y-4">
            <div
              className={`p-4 rounded-lg ${isCorrect ? "bg-green-100" : "bg-red-100"} animate-fadeIn`}
        ...
        {showHint && !submitted && (
          <div className="text-gray-600 text-sm p-3 bg-gray-100 rounded-lg">
            {item.hint ? (
              ...
              <p>
                {"Pista: La paraula te"} {item.correctForms[0].length} {"lletres i comença per '"}{item.correctForms[0][0]}{"'"}
              </p>
            )}
            {item.isSeptetFunest && (
              <p className="mt-1 text-red-600 font-medium">
                Aquesta paraula forma part del "Septet Funest" - els 7 errors mes comuns en catala!
              </p>
            )}
          </div>
        )}
