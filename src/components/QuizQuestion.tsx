...
export function QuizQuestion({ item, onAnswer, onContinue, onRestart, answered, isFirstQuestion = false }: QuizQuestionProps) {
  ...
  return (
    <div className="flex flex-col w-full max-w-md mx-auto p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md transition-colors duration-300">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">Corregeix aquest barbarisme:</h2>
        <div className="text-3xl font-bold text-red-600 dark:text-red-400 p-4 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg text-left md:text-center">
          {item.barbarism}
        </div>
      </div>
      ...
      <div className="flex justify-end mb-2">
        <button
          ...
          className="flex items-center gap-1 text-sm bg-white dark:bg-gray-700 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-700 dark:hover:text-red-300 hover:border-red-300 py-1.5 px-3 rounded-lg shadow-sm transition duration-200"
      ...
      {showRestartConfirm && (
        <div className={`modal-container bg-black bg-opacity-50 p-4 ${isExitingModal ? 'exiting' : ''}`}>
          <div className={`modal-content bg-white dark:bg-gray-800 rounded-lg p-5 max-w-xs w-full shadow-lg ${isExitingModal ? 'exiting' : ''}`}>
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-3">Confirmar reinici</h3>
            <p className="text-gray-600 dark:text-gray-300 mb-4">Estas segur que vols reiniciar el quiz? Perdras tot el progres actual.</p>
            <div className="flex gap-3">
              <button
                onClick={handleCloseModal}
                className="flex-1 py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300"
              >
                Cancel·lar
              </button>
              <button
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input
            ...
            className={`w-full px-4 py-3 rounded-lg border ${
              submitted
                ...
                : 'border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-red-500 focus:ring-red-300'
        ...
        {!submitted ? (
          ...
          <div className="space-y-4">
            <div className={`p-4 rounded-lg ${isCorrect ? 'bg-green-100 dark:bg-green-900/40' : 'bg-red-100 dark:bg-red-900/40'} animate-fadeIn`}
              ...
              {!isCorrect && (
                <div className="text-gray-700 dark:text-gray-300">
                  <p className="mb-1">Les formes correctes son:</p>
        ...
        {showHint && !submitted && (
          <div className="text-gray-600 dark:text-gray-300 text-sm p-3 bg-gray-100 dark:bg-gray-700 rounded-lg">
            {item.hint ? (
              <p>Pista: {item.hint}</p>
            ) : (
              <p>Pista: La paraula te {item.correctForms[0].length} lletres i comenca per '{item.correctForms[0][0]}'</p>
            )}
            {item.isSeptetFunest && (
              <p className="mt-1 text-red-600 dark:text-red-400 font-medium">Aquesta paraula forma part del "Septet Funest" - els 7 errors mes comuns en catala!</p>
            )}
          </div>
        )}
      </form>
