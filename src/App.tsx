...
export function App() {
  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 bg-gradient-main flex flex-col items-center justify-center transition-all duration-300" style={{ fontFamily: 'Poppins, sans-serif' }}>
        <div className="relative flex items-center justify-center w-16 h-16">
          <div className="absolute h-16 w-16 rounded-full animate-spin bg-gradient-to-b from-red-600 to-transparent"></div>
          <div className="absolute flex items-center justify-center bg-white dark:bg-slate-800 rounded-full h-14 w-14 transition-colors duration-300">
        ...
        <h1 className="mt-6 text-2xl font-bold text-red-800 dark:text-red-400 transition-colors duration-300">Mots Correctes</h1>
        <p className="text-red-600 dark:text-red-300 mt-2 transition-colors duration-300">Preparant el teu quiz...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-main py-6 md:py-8 px-4 transition-all duration-300" style={{ fontFamily: 'Poppins, sans-serif' }}>
      <div className="container mx-auto">
        <header className="flex flex-col items-center justify-between mb-6 md:mb-8">
          <div className="flex items-center mb-4 w-full justify-center relative">
            <div className="text-center">
              <h1 className="text-2xl md:text-3xl font-bold text-red-800 dark:text-red-400 transition-colors duration-300">Estudiar Català CSI</h1>
              <p className="text-sm md:text-base text-red-600 dark:text-red-300 transition-colors duration-300">SACA UN 10 EN CATALAN</p>
            </div>
          ...
          <div className="flex items-center justify-center flex-row space-x-2 md:space-x-4 w-full md:w-auto">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-300 ${
                isStudyMode 
                  ? 'bg-red-600 dark:bg-red-500 text-white' 
                  : 'bg-white dark:bg-slate-700 text-red-600 dark:text-red-300 hover:bg-red-50 dark:hover:bg-slate-600'
              }`}
            >
              <BookOpen size={18} />
            ...
            <>
              {isStudyMode ? (
                ...
                <>
                  {!quizState.completed ? (
                    <>
                      <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 mb-6 transition-colors duration-300">
                        <QuizProgress 
        ...
        <footer className="text-center mt-8 md:mt-12 flex flex-col items-center">
          <OfflineButton />
          <p className="mt-4 text-sm text-red-400 dark:text-red-300 text-left md:text-center transition-colors duration-300">Català CSI - Por Víctor De Nadal - Colegio San Ignacio (CSI) Barcelona</p>
        </footer>
      </div>
    </div>
  );
